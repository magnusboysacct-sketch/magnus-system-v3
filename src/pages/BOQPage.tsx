...
export default function BOQPage() {
  ...
  async function saveBoqToSupabase(nextStatus: "draft" | "approved") {
    ...
    try {
      ...
      // Step 5: Insert sections WITHOUT client IDs
      // Let Postgres generate fresh UUIDs to avoid FK violations
      const sectionIdMapping = new Map<string, string>(); // old client ID -> new database ID

      if (sections.length > 0) {
        const sectionPayload = sections.map((s, idx) => ({
          // DO NOT include id - let Postgres generate it
          boq_id: headerId,
          sort_order: idx,
        ...
        console.log("[BOQ Save] Inserting", sectionPayload.length, "sections (Postgres will generate IDs)...");
        ...
        if (!insertedSections || insertedSections.length === 0) {
          throw new Error("Failed to get inserted section IDs from database");
        }

        // Build mapping: client section ID -> database section ID (by sort_order)
        insertedSections.forEach((dbSection) => {
          const clientSection = sections[dbSection.sort_order];
          if (clientSection) {
            sectionIdMapping.set(clientSection.id, dbSection.id);
            console.log("[BOQ Save] Section ID mapping:", clientSection.id, "->", dbSection.id);
          }
        });

        console.log("[BOQ Save] ✓ Sections inserted with database UUIDs:", insertedSections.map(s => s.id));
      } else {
        console.log("[BOQ Save] No sections to insert");
      }

      // Step 6: Insert items WITHOUT client IDs, using mapped section IDs
      const itemPayload: any[] = [];
      for (const s of sections) {
        const dbSectionId = sectionIdMapping.get(s.id);
        if (!dbSectionId) {
          console.error("[BOQ Save] ERROR: No database ID found for section:", s.id);
          throw new Error(`Section ID mapping failed for section: ${s.title}`);
        }

        for (let i = 0; i < s.items.length; i++) {
          const it = s.items[i];
          const payload = {
            // DO NOT include id: it.id - let Postgres generate it
            section_id: dbSectionId, // Use the mapped database section ID
            sort_order: i,
      ...
      if (itemPayload.length > 0) {
        console.log("[BOQ Save] Inserting", itemPayload.length, "items (Postgres will generate IDs)...");
        itemPayload.slice(0, 3).forEach((item, idx) => {
        ...
        console.log("[BOQ Save] ✓ Items inserted with database UUIDs");
        console.log("[BOQ Save] Sample inserted item IDs:", insertedItems?.slice(0, 3).map(i => `${i.item_name}: ${i.id}`));
      } else {
      ...
      console.log("[BOQ Save] Reloading BOQ to sync React state with database IDs...");
      await loadLatestBoqForProject(projectId);
      console.log("[BOQ Save] BOQ reloaded - React state now has real database UUIDs");
 ...
 useEffect(() => {
  ...
  async function handleGenerateProcurement() {
    ...
    try {
      ...
      console.log("[BOQ → Procurement] STARTING PROCUREMENT GENERATION FLOW");
      console.log("=".repeat(80));
      console.log("[BOQ] Step 1: Saving BOQ to ensure all items have database IDs...");
      console.log("[BOQ]   Current BOQ ID:", boqId);
      console.log("[BOQ]   Project ID:", routeProjectId);
      console.log("[BOQ]   React state sections:", sections.length);
      console.log("[BOQ]   React state total items:", sections.reduce((sum, s) => sum + s.items.length, 0));

      // Force save the BOQ to ensure all items have database IDs
      // This will:
      // 1. Delete old sections/items
      // 2. Insert new sections (Postgres generates UUIDs)
      // 3. Insert new items (Postgres generates UUIDs)
      // 4. Reload BOQ from database to sync React state
      await saveBoqToSupabase("draft");

      console.log("\n[BOQ] Step 2: Verifying BOQ has valid database IDs...");
      console.log("[BOQ]   BOQ ID after save:", boqId);
      console.log("[BOQ]   Sections in React state:", sections.length);
      console.log("[BOQ]   Sample section IDs:", sections.slice(0, 2).map(s => s.id));
      console.log("[BOQ]   Sample item IDs:", sections[0]?.items.slice(0, 2).map(i => i.id));

      // Validate that we have database UUIDs, not client-generated IDs
      const hasTemporaryIds = sections.some(s =>
        s.id.startsWith('id_') || s.items.some(i => i.id.startsWith('id_'))
      );

      if (hasTemporaryIds) {
        console.error("[BOQ] ERROR: Found temporary client IDs after save!");
        setPersistError("BOQ save incomplete. Please try saving again before generating procurement.");
        return;
      }

      console.log("[BOQ] ✓ All IDs are valid database UUIDs");

      console.log("\n[BOQ] Step 3: Reloading BOQ one more time to ensure fresh data...");

      // Extra reload to be absolutely certain we have fresh database data
      await loadLatestBoqForProject(routeProjectId);

      console.log("[BOQ] ✓ BOQ reloaded from database");

      console.log("\n[BOQ] Step 4: Generating procurement from saved BOQ...");
      console.log("[BOQ]   This will fetch fresh data from database, not React state");
