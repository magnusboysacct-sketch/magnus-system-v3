...
const MS_STATUS_CFG: Record<string,{color:string;bg:string;border:string;label:string}> = {
  planned:  {color:"text-slate-400",  bg:"bg-slate-500/10",  border:"border-slate-500/20",  label:"Planned"},
...
const TASK_STATUS_CFG: Record<string,{color:string;bg:string;border:string}> = {
  planned:  {color:"text-slate-400",  bg:"bg-slate-500/10",  border:"border-slate-500/20"},
      ...
      <div className="border-b border-white/[0.06] bg-[#0d1117] px-6 py-4">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={()=>nav("/projects")} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-slate-600 hover:text-slate-300 transition flex-shrink-0">
              <ChevronLeft size={16}/>
            </button>
      ...
      <div className="p-6 space-y-5">{/* ── OVERVIEW ── */}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {[
                ...
                <div key={card.label} className={`rounded-xl border ${card.border} ${card.bg} p-4`}>
                  ...
                  <div className="text-[10px] text-slate-700 mt-0.5">{card.sub}</div>
                </div>
              ))}
            </div>
            ...
            {milestones.length>0&&(
              <div className="rounded-xl border border-white/[0.07] bg-[#0d1117] p-5">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <div className="text-sm font-bold text-slate-200">Overall Project Progress</div>
                    <div className="text-[10px] text-slate-600 mt-0.5">{msComplete} of {milestones.length} milestones complete</div>
                  </div>
              ...
              <div className="rounded-xl border border-white/[0.07] bg-[#0d1117] p-5">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    {label:"Milestones",value:<span className="text-xs text-slate-300 font-semibold">{milestones.length}</span>},
                    {label:"Tasks",   value:<span className="text-xs text-slate-300 font-semibold">{tasks.length}</span>},
        ...
        {tab==="milestones"&&(
          <>
            ...
            <div className="grid grid-cols-4 gap-3">
              {[
                {label:"Total",    value:milestones.length,                              color:"text-slate-300",   bg:"bg-white/[0.04]",   border:"border-white/[0.07]"},
                {label:"Planned",  value:milestones.filter(m=>m.status==="planned").length,  color:"text-slate-400",   bg:"bg-slate-500/10",   border:"border-slate-500/20"},
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-slate-200">Project Milestones</span>
            ...
            {showMSForm&&(
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                <div className="w-full max-w-lg rounded-2xl border border-white/[0.08] bg-[#0d1117] shadow-2xl max-h-[90vh] overflow-y-auto">
                  <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.07] sticky top-0 bg-[#0d1117]">
                    ...
                    <button onClick={()=>setShowMSForm(false)} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-slate-500"><X size={14}/></button>
                  </div>
                  <div className="p-5 space-y-3">
                    <div>
                      <label className="text-[9px] font-bold uppercase tracking-widest text-slate-600 block mb-1">Milestone Name *</label>
                    ...
                    <div>
                      ...
                      <textarea value={msForm.description} onChange={e=>setMsForm(f=>({...f,description:e.target.value}))}
                        ...
                        className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-sm text-slate-200 placeholder-slate-700 outline-none focus:border-cyan-500/50 resize-none"/>
                    </div>
                    <div>
                    ...
                    <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-3 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="text-[9px] font-bold uppercase tracking-widest text-slate-600 block mb-1">Planned Start</label>
                        ...
                        <div>
                          <label className="text-[9px] font-bold uppercase tracking-widest text-slate-600 block mb-1">Planned End</label>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="text-[9px] font-bold uppercase tracking-widest text-slate-600 block mb-1">Crew Size Needed</label>
                          <input type="number" value={msForm.crew_size_needed} onChange={e=>setMsForm(f=>({...f,crew_size_needed:e.target.value}))}
                            placeholder="e.g. 5"
                            className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-slate-200 placeholder-slate-700 outline-none focus:border-cyan-500/50"/>
                        </div>
                        <div>
                          <label className="text-[9px] font-bold uppercase tracking-widest text-slate-600 block mb-1">Production Rate/Day</label>
                    ...
                    {/* Budget */}
                    <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-3 space-y-3">
                      <div className="text-[10px] font-bold uppercase tracking-widest text-slate-600">Planned Budget (JMD)</div>
                      <div className="grid grid-cols-3 gap-3">
                        <div>
                          <label className="text-[9px] font-bold uppercase tracking-widest text-slate-600 block mb-1">Labour</label>
                          <input type="number" value={msForm.planned_labour_cost} onChange={e=>setMsForm(f=>({...f,planned_labour_cost:e.target.value}))}
                            placeholder="0"
                            className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-slate-200 placeholder-slate-700 outline-none focus:border-cyan-500/50"/>
                        </div>
                        <div>
                          <label className="text-[9px] font-bold uppercase tracking-widest text-slate-600 block mb-1">Materials</label>
                          <input type="number" value={msForm.planned_material_cost} onChange={e=>setMsForm(f=>({...f,planned_material_cost:e.target.value}))}
                            placeholder="0"
                            className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-slate-200 placeholder-slate-700 outline-none focus:border-cyan-500/50"/>
                        </div>
                        <div>
                          <label className="text-[9px] font-bold uppercase tracking-widest text-slate-600 block mb-1">Equipment</label>
                    ...
                    <div>
                      <label className="text-[9px] font-bold uppercase tracking-widest text-slate-600 block mb-1">Contract Payment Amount (JMD)</label>
            ...
            {milestones.length===0?(
              <div className="flex flex-col items-center justify-center py-16 gap-3 text-center rounded-xl border border-white/[0.07] bg-[#0d1117]">
                ...
                <p className="text-slate-400 text-sm font-medium">No milestones yet</p>
                <p className="text-slate-700 text-xs">Break your project into phases — Foundation, Superstructure, Roofing...</p>
                <button onClick={openNewMS}
              ...
              <div className="flex flex-col gap-4">
                {milestones.map(ms=>{
                  ...
                  return(
                    <div key={ms.id} className="rounded-xl border border-white/[0.07] bg-[#0d1117] overflow-hidden">
                      {/* Milestone Header */}
                      <div className="p-4 flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-[10px] font-bold text-slate-600">#{ms.milestone_no}</span>
                            <span className="text-sm font-bold text-slate-200">{ms.milestone_name}</span>
                            <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold border ${cfg.color} ${cfg.bg} ${cfg.border}`}>{cfg.label}</span>
                          </div>
                          {ms.description&&<p className="text-[11px] text-slate-600 mb-2">{ms.description}</p>}
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          ...
                          <button onClick={()=>openEditMS(ms)} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-slate-600 hover:text-slate-300 transition"><Edit2 size={12}/></button>
                          <button onClick={()=>deleteMS(ms.id)} className="p-1.5 rounded-lg hover:bg-red-500/10 text-slate-600 hover:text-red-400 transition"><Trash2 size={12}/></button>
                        </div>
                      </div>

                      {/* Budget Row */}
                      {ms.planned_total_cost>0&&(
                        <div className="px-4 py-2 border-t border-white/[0.04] grid grid-cols-3 gap-4 bg-white/[0.01]">
                          {[
                            ...
                            <div key={b.label} className="text-center">
                              ...
                              <div className="text-[9px] text-slate-700 uppercase tracking-wider">{b.label}</div>
                            </div>
                          ))}
                        </div>
                      )}
                      ...
                      {msTasks.length>0&&(
                        <div className="border-t border-white/[0.04]">
                          <div className="px-4 py-2 flex items-center justify-between">
                            <span className="text-[9px] font-bold uppercase tracking-widest text-slate-600">Tasks ({msTasksDone}/{msTasks.length} done)</span>
                          </div>
                          {msTasks.map(t=>{
                            ...
                            return(
                              <div key={t.id} className="flex items-center gap-3 px-4 py-2.5 border-t border-white/[0.03] hover:bg-white/[0.02] transition">
                                <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold border ${tcfg.color} ${tcfg.bg} ${tcfg.border}`}>{t.status}</span>
                                <div className="flex-1 min-w-0">
                                  <div className="text-xs font-semibold text-slate-300">{t.task_name}</div>
                                  {t.trade_type&&<div className="text-[9px] text-slate-600">{t.trade_type}</div>}
                                </div>
                                {t.quantity&&<span className="text-[10px] text-slate-600">{t.quantity} {t.unit||""}</span>}
                                {tTotal>0&&<span className="text-xs font-bold text-emerald-400">{fmtJMD(tTotal)}</span>}
                                <div className="flex items-center gap-1">
                                  <button onClick={()=>openEditTask(t)} className="p-1 rounded hover:bg-white/[0.06] text-slate-600 hover:text-slate-300"><Edit2 size={10}/></button>
                                  <button onClick={()=>deleteTask(t.id)} className="p-1 rounded hover:bg-red-500/10 text-slate-600 hover:text-red-400"><Trash2 size={10}/></button>
                                  <select value={t.status} onChange={e=>updateTaskStatus(t.id,e.target.value)}
                                    className="text-[9px] bg-[#080b10] border border-white/[0.07] rounded px-1 py-0.5 text-slate-500 outline-none">
                                    <option value="planned">Planned</option>
                                    <option value="active">Active</option>
                                    <option value="complete">Complete</option>
                                  </select>
                      ...
                      <div className="px-4 py-2.5 border-t border-white/[0.04] flex items-center justify-between bg-white/[0.01]">
                        <span className="text-[10px] text-slate-600">{msTasks.length} tasks · {msTasksDone} complete</span>
        ...
        {(tab==="tasks"||showTaskForm)&&(
          <>
            <div className="grid grid-cols-4 gap-3">
              {[
                {label:"Total",    value:tasks.length,                                color:"text-slate-300",   bg:"bg-white/[0.04]",   border:"border-white/[0.07]"},
                {label:"Planned",  value:tasks.filter(t=>t.status==="planned").length, color:"text-slate-400",   bg:"bg-slate-500/10",   border:"border-slate-500/20"},
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-slate-200">All Tasks</span>
            ...
            {showTaskForm&&(
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                <div className="w-full max-w-lg rounded-2xl border border-white/[0.08] bg-[#0d1117] shadow-2xl max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="p-5 space-y-3">
                    <div>
                      <label className="text-[9px] font-bold uppercase tracking-widest text-slate-600 block mb-1">Milestone</label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="date" value={taskForm.start_date} onChange={e=>setTaskForm(f=>({...f,start_date:e.target.value}))}
                          className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-slate-200 outline-none"/>
                      </div>
                      <div>
            ...
            {tasks.length===0?(
              <div className="flex flex-col items-center justify-center py-16 gap-3 text-center rounded-xl border border-white/[0.07] bg-[#0d1117]">
                ...
                <p className="text-slate-700 text-xs">Add milestones first, then add tasks to each milestone</p>
                ...
                {tasks.map(task=>{
                  ...
                  return(
                    <div key={task.id} className="rounded-xl border border-white/[0.07] bg-[#0d1117] p-4">
                      ...
                      {task.quantity&&(
                        <div className="flex items-center gap-4 text-xs mb-2 p-2 rounded-lg bg-white/[0.02] border border-white/[0.04]">
        ...
        {tab==="financials"&&(
          <>
            ...
            {milestones.length>0&&(
              <div className="rounded-xl border border-white/[0.07] bg-[#0d1117] overflow-hidden">
                <div className="px-5 py-3 border-b border-white/[0.06]">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-600">Cost per Milestone</span>
                </div>
                {milestones.map((ms,i)=>{
                  const pct=ms.planned_total_cost>0?Math.min(100,(ms.actual_total_cost/ms.planned_total_cost)*100):0;
                  return(
                    <div key={ms.id} className={`px-5 py-4 border-b border-white/[0.04] last:border-0 ${i%2===1?"bg-white/[0.01]":""}`}>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-semibold text-slate-200">{ms.milestone_name}</span>
