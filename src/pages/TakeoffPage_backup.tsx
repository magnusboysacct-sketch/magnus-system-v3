...
function TakeoffPageInner() {
  ...
  async function fetchCostItem(itemId: string) {
    try {
  ...
  function calculateQuantity(measurement: Measurement, costItem: any) {
    if (!costItem?.calc_engine_json) {
      return measurement.result;
    }

    try {
      const calcJson = typeof costItem.calc_engine_json === 'string' 
    ...
    if (tool === "line") {
      ...
      if (!lineEnd) {
        ...
        if (feetPerPixel && feetPerPixel > 0) {
          ...
          let costItem = activeCostItem;
          if (activeGroupId && !costItem) {
            costItem = await fetchCostItem(activeGroupId);
            setActiveCostItem(costItem);
          }
          ...
          const calculatedQuantity = calculateQuantity(newMeasurement, costItem);
          
          // Update the measurement with calculated quantity
          updateMeasurement(newMeasurement.id, { result: calculatedQuantity });
        }
  ...
  async function confirmVolumeWithDepth() {
    if (volumePoints.length < 3) return;
