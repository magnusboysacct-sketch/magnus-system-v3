...
export function BOQSuggestionCard({ suggestion, onAdd, onIgnore, isAdding }: BOQSuggestionCardProps) {
  return (
    <div className="bg-slate-100 dark:bg-slate-800/50 rounded-lg p-4 border border-slate-300 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-600 transition-colors">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start gap-2 mb-3">
            <Sparkles className="w-3 h-3 text-purple-600 dark:text-purple-400 flex-shrink-0 mt-0.5" />
            <p className="text-xs text-slate-600 dark:text-slate-400 flex-1">{suggestion.reason}</p>
          </div>
          ...
          {suggestion.relatedTo && suggestion.relatedTo.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-3">
              {suggestion.relatedTo.map((related) => (
                <span
                  key={related}
                  className="text-xs px-2 py-0.5 rounded bg-blue-500/10 text-blue-700 dark:text-blue-400 border border-blue-500/30"
                >
                  Related to: {related}
                </span>
              ))}
          ...
          {suggestion.isAssembly && suggestion.assemblyItems && suggestion.assemblyItems.length > 0 && (
            <div className="mb-3 bg-slate-50 dark:bg-slate-900/50 rounded p-2 border border-slate-200 dark:border-slate-700">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={isAdding}
              className="px-3 py-2 rounded-lg bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Ignore
            </button>
          </div>
